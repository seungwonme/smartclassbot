...
interface BrandContentReviewTabProps {
  campaignId: string;
  confirmedInfluencers: any[];
  toast: any;
}

const BrandContentReviewTab: React.FC<BrandContentReviewTabProps> = ({
  campaignId,
  ...
  const loadContentReviews = async () => {
    try {
      setIsLoading(true);
      console.log('🎯 브랜드 관리자 - 콘텐츠 검수 데이터 로딩');
  ...
  const handleApproveContent = async (contentId: string) => {
    try {
      await contentReviewService.approveContent(campaignId, contentId);
      ...
      if (selectedContent?.id === contentId) {
        setSelectedContent(prev => prev ? { ...prev, reviewStatus: 'approved' } : null);
      }

      toast({
        title: "콘텐츠 승인 완료",
        description: "콘텐츠가 성공적으로 승인되었습니다."
      });

      // 모든 콘텐츠가 승인되었는지 확인
      const updatedReviews = contentReviews.map(content =>
        content.id === contentId 
          ? { ...content, reviewStatus: 'approved' }
          : content
      );
      
      const allApproved = updatedReviews.every(content => content.reviewStatus === 'approved');
      if (allApproved) {
        toast({
          title: "캠페인 라이브 시작",
          description: "모든 콘텐츠가 승인되어 캠페인이 라이브 상태로 전환됩니다.",
          duration: 5000
        });
      }
      ...
      toast({
        title: "승인 실패",
        description: "콘텐츠 승인에 실패했습니다.",
        variant: "destructive"
      });
    }
  };

  const handleRequestRevision = async (contentId: string, feedback: string) => {
    try {
      const updatedContent = await contentReviewService.requestContentReview(campaignId, contentId, feedback);
      ...
      toast({
        title: "수정요청 전송 완료",
        description: "시스템 관리자에게 콘텐츠 수정요청이 전송되었습니다."
      });

    } catch (error) {
      console.error('수정요청 전송 실패:', error);
      toast({
        title: "전송 실패",
        description: "수정요청 전송에 실패했습니다.",
        variant: "destructive"
      });
    }
  };
    ...
    return (
      <Card>
        <CardContent className="text-center py-12 text-gray-500">
          ...
          <p className="text-sm mt-2">시스템 관리자가 콘텐츠를 업로드하면 여기에 표시됩니다.</p>
        </CardContent>
      </Card>
    );
  }
...
export default BrandContentReviewTab;
